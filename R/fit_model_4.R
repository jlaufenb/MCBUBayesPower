...
#'
#' @return  JAGS MCMC output from fitting an exponential growth model with individual birds as observation unit and observer random effect on sigma parameter to MCBU distance-sampling data from 2003 and 2018. Output is saved as an RData file named \code{out4.RData}.
#' @export
#'
fit_model4 <- function(mcbu_data_file = "distdata_mcbu.csv", ni = 200, nb = 100, nt = 1, nc = 1, M = c(200,100),
    ...
    if(save_output) save(out4, file = paste0(jags_output_folder, "/out4.RData"))
    return(out4)
}
