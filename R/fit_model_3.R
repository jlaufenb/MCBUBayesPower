...
#'
#' @return  JAGS MCMC output from fitting an exponential growth model with individual groups as observation unit and observer random effect on sigma parameter to MCBU distance-sampling data from 2003 and 2018
#' @export
#'
fit_model3 <- function(mcbu_data_file = "distdata_mcbu.csv", ni = 200, nb = 100, nt = 1, nc = 1, M = c(200,100),
    ...
    if(save_output) save(out3, file = paste0(jags_output_folder, "/out3.RData"))
    return(out3)
}
